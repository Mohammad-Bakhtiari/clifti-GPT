...
- ``communication_cost_table_*.tex`` — longtables (one per workflow; ``\\input`` these directly)
...
def write_latex_tables(df: pd.DataFrame, out_dir: Path) -> None:
    """Render one longtable per workflow into *out_dir*."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Directory for communication_cost_table_*.tex fragments.",
    )
    p.add_argument(
        "--latex_macros",
        type=str,
...
def main() -> None:
    ...
    if not macros_path.is_absolute():
        macros_path = _REPO_ROOT / macros_path
    write_latex_tables(df, out_dir)
