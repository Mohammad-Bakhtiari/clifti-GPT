...
def _wrap_cell(width: str, content: str, halign: str = "left") -> str:
    ...
    return (
        rf"\begin{{minipage}}[t]{{{width}}}{align_cmd} {content}\end{{minipage}}"
    )
...
def _config_multirow_cell(config: str, n_rows: int) -> str:
    body = (
        rf"\begin{{minipage}}[t]{{{CONFIG_COL_WIDTH}}}\raggedright {config}\end{{minipage}}"
    )
    ...
    for group_idx, (payload, group) in enumerate(groups):
        ...
        for row_idx, (_, row) in enumerate(group.iterrows()):
            ...
            if row_idx == 0:
                ...
                config_cell = _wrap_cell(CONFIG_COL_WIDTH, "", "left")
